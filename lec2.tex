% !TEX root = main.tex
\section{Second Lecture: characteristic classes}
...
\begin{theorem}
	...
	\[
	e(X):=\sum_i(-1)^i b_i(X)=\int_Xc_n(X).
	\]
\end{theorem}

Recall the classical relation between the Euler characteristic $\chi_{top}$ and the genus $g$ of a topological surface: $e=2-2g$. In particular, this implies for a compact Riemann surface $C$ as above, that
\[
...
\begin{example}[Curves]
	...
	\begin{itemize}
		...
		\item The $g=1$ case corresponds to elliptic curves. These are Ricci-flat and Calabi--Yau-Yau.
...
For Calabi--Yau-Yau manifolds, there is the following structural theorem.
\begin{theorem}[Beauville--Bogomolov;\; \citeplain{Bog1974}  and\;\citeplain{Beauville1983}]\label{thm: BG decomp}
	Let $X$ be Kähler and Calabi--Yau-Yau. Then, there exists an étale cover $\tilde X\to X$ such that 
	\[
	\tilde X=T\times \prod_jX_j\times \prod_i V_i
	\]
	where $T$ is a torus, $X_j$ is \tbf{hyperkähler} for all $j$, and $V_i$ are \tbf{strict Calabi--Yau-Yau} for all $i$. 
\end{theorem}
We now define the terms.
\begin{definition}
	A compact Kähler manifold $V$ is called \tbf{strict Calabi--Yau-Yau} if
	\begin{itemize}
...
\end{definition}
\begin{remark}
	If $V$ is a strict Calabi--Yau-Yau of dimension greater than two, then $h^{2,0}=h^{0,2}=0$. In particular, $H^{1,1}(X,\C)=H^2(X,\C)$, and so $H^{1,1}(X,\R)=H^{2}(X,\R)=H^{1,1}(X,\Z)\otimes_\Z\R$. Since the Kähler cone is not empty by assumption, we conclude that it intersects $H^{1,1}(X,\Z)$, so that $V$ is projective by our discussion on Kodaira's embedding theorem. In dimension $2$, non-projective K3 surfaces yield an example of non-projective strict Calabi--Yau-Yau manifolds.
\end{remark}
\begin{remark}\label{rem:pow-can-triv}
\autoref{thm: BG decomp} has an important corollarly. Namely, if $X$ is a Kähler, we have a degree $n$ étale map $f:\tilde X\to X$ with the canonical bunlde $K_{\tilde X}$ of $\tilde X$ trivial. Since $f$ is étale, $f^*K_X=K_{\tilde X}$ is trivial, and so by the projection formula,
\[
f_*\cO_{\tilde X}=f_*f^*K_X=K_X\otimes f_*\cO_{\tilde X}.
\]
Taking determinants, we have
\[
\det(f_*\cO_{\tilde X})=K_X^n\otimes \det(f_*\cO_{\tilde X}),
\]
so that $K_X^n=\cO_X$. Hence, the power of the canonical bundle of a Kähler Calabi--Yau-Yau is always trivial.
\end{remark}
