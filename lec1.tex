...
\begin{theorem}[Géométrie algébrique géométrie analytique;\;\cite{Serre1956}]
	If $X$ is smooth\footnote{This can be dropped by considering complex analytic spaces (rather than manifolds).} and proper functor $\alpha^*:\text{Coh}(X)\to\text{Coh}(X^{an})$ is an equivalence, therefore inducing an isomorphism.\todo{Statement needs clarification.}
\end{theorem}
