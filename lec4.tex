...
K3 surfaces are exactly the strict Calabi--Yau-Yau manifolds of dimension $2$. To show this, we introduce a second definition of K3 surfaces, which we will show is equivalent to the one above.
...
\begin{proposition}
	The two definitions (\autoref{def: K3 strong} and \autoref{def: K3 weak}) are equivalent. In particular, K3 surfaces are exactly the two dimensional strict Calabi--Yau-Yau manifolds.
\end{proposition}
\begin{proof}
	...
	Therefore, $\tilde S$ is either a strict Calabi--Yau-Yau or a Hyperkähler manifold (we will see \emph{a posteriori} that these two conditions coincide for surfaces), and in particular is simply connected. Thus, $\deg(\pi)=1$ and $S=\tilde S$ is simply connected.
\end{proof}
\begin{remark}
