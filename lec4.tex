...
K3 surfaces are exactly strict Calabi-Yau manifolds of dimension $2$. To show this, we introduce a second definition of K3 surfaces, which we will show are equivalent.
...
\begin{proposition}
	The two definitions (\autoref{def: K3 strong} and \autoref{def: K3 weak}) are equivalent. In particular, K3 surfaces are exactly strict Calabi-Yau manifolds surfaces.
\end{proposition}
\begin{proof}
	...
	We apply the Beauville-Bogomolov decomposition theorem (\autoref{thm: BG decomp}) to conclude that there exists an étale cover $\pi: \tilde{S}\to S$ where $\tilde S$ is either a complex torus, a Hyperkähler surface,  a strict Calabi-Yau surface, or a product of two elliptic curves. Suppose first that $\tilde S$ is either a product of two elliptic curves or of a complex torus. Then,
	%
	\[
...
\end{proof}
\begin{remark}
	Importantly, it is also true that K3 surfaces are exactly Hyperkähler manifolds of dimension $2$, but we shall see this later.
\end{remark}
...
\begin{proof}
	...
	\[
	\chi_{top}(S) = \sum (-1)^i b_i(S) = 1-0+b_2(S)-0+1 = 2+b_2(S).
	\]
	By the Gauss-Bonnet theorem, we have 
	\[
	\chi_{top}(S)=\int_S c_2(S).
	\]
...
\begin{remark}
	...
	Note that not every finite covering $f:X\to Y$ of degree $m$ that is a cyclic covering. However, in characteristic coprime to $m$, we can say the following.  The exact sequence 
	\[
...
\begin{example}[Double cover of $\bP^2$ branched over a sextic]\label{eg: double cover K3}
	...
	Since $f$ is finite, 
	\[
	H^1(X,\cO_X)\simeq H^1(\bP^2,f*\cO_X)=H^1(\bP^2,\cO_{\bP^2})\oplus H^1(\bP^2,\cO(-3))=0.
	\]
...
\begin{example}[Kummer surfaces]\label{eg: Kummer K3}
	...
	Incidentally, the fixed locus of $\tilde{i}$ is exactly the union of the exceptional divisors $\bigcup E_i$. This implies that the quotient $X=\tilde A/\tilde i$, which we define to be the \tbf{Kummer surface} of $A$, is smooth because the fixed locus has codimension $1$. $X$ is Kähler since it isthe quotient of a Kähler surface by a finite group. In order to check that it is a K3 surface, there are two conditions to check: that the canonical bundle is trivial, and that $H^1(X,\cO_X)=0$.
...
\begin{remark}
	Note that the map $q:A\to A/i$ is flat. Let $Z$ be the singular locus of $A/i$, comprising of 16 points. Recall that the blow-up (i.e. Rees) algebra of $A$ at $Z$ is
	\[
	\cO_{A/i}[\cI_Zt],
	\]
	where $\cI_Z$ is the ideal at $Z$. We can equally recover the blow-up from the Rees algebra in the analytic setting, where there exists an analytic version of the projective spectrum; see \cite{Aroca2018}. Now, since $q$ is flat,
	\[
	q^*\cO_{A/i}[\cI_Zt]=\cO_{A}[(q^*\cI_Z)t]=\cO_A[\cI_{Z'}t],
	\] 
	where $Z'=q^{-1}(Z)$, that is, $Z'$ consists of the 16 points fixed by $i$. The projective spectrum commutes with base change, and so if $\widetilde{A/i}$ denotes the blow up of $A/i$ at $Z$, we have a cartesian diagram
	\[
	\begin{tikzcd}
		\tilde A\ar[r]\ar[d]&\widetilde {A/i}\ar[d]\\
		A\ar[r,"q"]&A/i
	\end{tikzcd}
	\] 
	now the quotient $q:A\to A/i$ may locally be defined as a GIT quotient, (we are quotienting by a finite group), and so it is universal under base change, hence $\tilde A\to\widetilde{A/i}$ is also the quotient of $\tilde A$ by the lifted action. But this implies that $\tilde A/\tilde i=\widetilde{A/i}=X.$ That is, when constructing $X$, we may equally have taken the quotient first, and resolve the singularities afterwards.
\end{remark}
