...
&=i \brk{J(u)}^{1,0}=i J(u),
	...
	&= g(I-,-)=\omega_I(-,-),
	\end{align*}
	where the last line comes from the fact that $ig(-id -,-)$ is applied via our identifications to a $(0,1)$ vector, so that it does not contribute to the $(1,1)$ form that we get along \eqref{eq:contr-h1}.
