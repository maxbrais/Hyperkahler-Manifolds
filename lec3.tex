...
%
\begin{align}
	\label{eq:poinc-lefschetz-pairing} Q_k:H^k(X,\C)\times H^k(X,\C)&\to \C\\
...
\]
but the form $\alpha\omega^{n-k}\wedge\overline{\beta}$ is of degree $2n$ but not of type $(n,n)$. Hence it vanishes, implying that the integral vanishes.
...
\end{proof}	
%
\begin{corollary}[Hodge index theorem]\label{cor:hdg-ind}
	Let $X$ be a compact Kähler manifold. Then, the signature of the Poincaré intersection pairing $Q_2$ on $H^2(X,\R)$ is 
	\[
	(2h^{2,0}+1,h^{1,1}-1).
	\]
\end{corollary}
\begin{proof}
	Let $\omega$ be a Kähler form. By the Lefschetz decomposition, we have
	\[
	H^2(X,\R)= \R\omega\oplus H^2(X,\R)_{prim}.
	\]
	Now, by definition, this decomposition is orthogonal for $Q_2$ and  since $\omega$ is Kähler, $Q_2(\omega,\omega)>0$.
	
	Let $\alpha\in H^2(X,\R)_{prim}$.  Since $\alpha$ is real, we have
	\[
	\alpha=\alpha^{2,0}+\alpha^{1,1}+\overline{\alpha^{2,0}},
	\]
	where $\alpha^{1,1}$ is real.
	
	We thus have a decomposition
	\[
	H^{2}(X,\R)_{prim}=(H^{2,0}(X)\oplus H^{0,2}(X)\cap H^2(X,\R))\oplus H^{1,1}(X,\R)_{prim},
	\]
	which we already know is orthogonal with respect to $Q_2$.
	
	Now, for $\alpha\in (H^{2,0}(X)\oplus H^{0,2}(X))\cap H^2(X,\R)$, by considering types, we have
	%
	\[
	Q_2(\alpha,\alpha)=\int_X\alpha\wedge\alpha\wedge \omega^{n-2}=2\int_X\alpha^{2,0}\wedge\overline{\alpha^{2,0}}\wedge\omega^{n-2}>0
	\] 
	by \autoref{thm:hdg-rie} (3). On the other hand, is $\alpha\in H^{1,1}(X,\R)$, we have
	\[
	Q_2(\alpha,\alpha)=\int_X\alpha\wedge\alpha\wedge \omega^{2n-2}=\int_X\alpha\wedge\overline{\alpha}\wedge \omega^{n-2}<0
	\]
	by \autoref{thm:hdg-rie} (3). And so we conclude that the signature of $Q_2$ is 
	\[
	(2h^{2,0}+1,h^{1,1}-1).\qedhere
	\]
\end{proof}
%
...
\]	
is related to the Chern map $c_1:\Pic(X)\to H^2(X,\C)$: after composing $\delta$ with the map $H^2(X,\Z)\to H^2(X,\C)$ (which kills torsion), we have $\delta(\cL)=-c_1(\cL)$, where $\cL\in\Pic(X)$.
\begin{theorem}[Lefschetz theorem on $(1,1)$ classes]\label{thm:lef-11}
	If $X$ is compact Kähler, then the first chern map $c_1$ composed with $H^2(X,\Z)\to H^2(X,\C)$ is surjective onto $H^{1,1}(X,\Z)$.
\end{theorem}
\begin{proof}
	Note that the compositio is indeed valued in $H^{1,1}(X,\Z)$ by definition of the Chern class (indeed, the $(2,0)$ and $(0,2)$ part of the curvature of the Chern connection vanish). The map $H^2(X,\Z)\to H^2(X,\cO_X)$ factors as
	\begin{equation*}
	H^2(X,\Z)\to H^2(X,\C)\to H^{0,2}(X,\C)\simeq H^2(X,\cO_X),
	\end{equation*}
	%
	where the middle arrow is the projection onto the $(2,0)$ part.
This can be seen as follows: the map $\Z\to\cO_X$ factors as $\Z\to\C\to\cO_X$, and the map in cohomology $H^2(\C,X)\to H^2(X,\cO_X)$ may be computed by considering the map between the resolutions
\[
\begin{tikzcd}
	0\ar[r]&\C\ar[r]\ar[d]&\cA^1(\C)\ar[d]\ar[r,"d"]&\cA^2(\C)\ar[d]\ar[r,"d"]&\cdots\\
	0\ar[r]&\cO_X\ar[r]&\cA^{0,1}\ar[r,"\op"]&\cA^{0,2}\ar[r,"\op"]&\cdots,
\end{tikzcd}
\]
where the vertical arrows are projections.	Thus, the map $H^2(X,\Z)\to H^2(X,\cO_X)$  vanishes on $(1,1)$ classes, so that $(1,1)$ classes are in the kernel of this map, and equivalently in the image of $c_1=-\delta$. 
\end{proof}
%
\begin{definition}
	We define the \tbf{Néron-Severi} group of $X$, denoted $\NS(X)$, to be the image of $c_1$ in $H^2(X,\Z)$. We note that $\NS(X)/\text{torsion}=H^{1,1}(X,\Z)$, as $H^{1,1}(X,\Z)$ is defined to be the intersection of $H^{1,1}(X)$ with the image of $H^2(X,\Z)$ in $H^2(X,\C)$.
	
	We define the rank $\rho(X)$ of $\NS(X)$ (or equivalently of $H^{1,1}(X,\Z)$) to be the \tbf{Picard number}.  To motivate this terminology, note that there is an exact sequence
	\[
	0\to \Pic^0(X)\to\Pic(X)\to \NS(X)\to 0,
	\]
	where $\Pic^0(X)$ is the connected component of the identity.
\end{definition}
%
\begin{remark}
	The integral Hodge conjecture, which has been proven to be false, states that the map
	\begin{align*}
	\{\text{algebraic $\Z$-cycles of dimension $k$}\}&\to H^{(k,k)}(X,\Z)\\
	Z&\mapsto [Z]
	\end{align*}
	is surjective. Nevertheless, the Lefschetz theorem on $(1,1)$ classes shows that it is true for $k=1$. The rational Hodge conjecture, notoriously unsolved, asks whether taking this map over $\Q$-coefficients is surjective.
\end{remark}	
